# Andrei Kolenkov (Adefe)
### Software Engineer @ NVIDIA | Python & ML Developer

![Static Badge](https://img.shields.io/badge/Backend-red)
![Static Badge](https://img.shields.io/badge/Python-green)
![Static Badge](https://img.shields.io/badge/ML-blue)
![Static Badge](https://img.shields.io/badge/Linux-yellow)

Welcome to my digital workshop! I build high-performance backend systems, microservice infrastructure, AI/ML solutions. You can find my main hub and services at **[adefe.xyz](https://adefe.xyz)**.

---

## 🌐 Production & Pet Projects

Worlds of my design:

* **[🔍 AI Semantic Search Directory](https://adefe.xyz)** — Intelligent solution for indexing and semantic search of images and texts within file directories.
* **[🍓 Strawberry Project (VK MiniApp)](https://vk.com)** — Commercial AI text generation business application.
* **[⛈️ Harmful Cloud ML](https://harmful-cloud.adefe.xyz)** — Research platform for simulating social network commenting behavior using AI text generation.
* **[🫐 Blueberry Cookbook](https://blueberry.adefe.xyz)** — Smart culinary application with decoupled Next.js frontend, Python backend, and isolated DB monitoring (phpMyAdmin/Mongo Express).
* **[📻 Adefe Personal Radio Station](https://radio.adefe.xyz)** — Self-hosted non-commercial streaming platform broadcasting electronic music.
* * **[⛏️ BMSTU Minecraft Server](https://mc.adefe.xyz)** — Infrastructure and hosting for the official BMSTU Minecraft (v1.21.1) server.

---

## 🛠️ Technical Toolkit

* **Languages & Frameworks:** Python (Django, FastAPI), JavaScript
* **AI / ML / Data:** Semantic Search, LLM Text Generation, Vector Embeddings
* **Infrastructure & Ops:** Linux (ArchLinux), Nextcloud, CI/CD, Webpack

---

## 📬 Connect with Me

* **Personal Hub:** [adefe.xyz](https://adefe.xyz)
* **Professional:** [LinkedIn](https://linkedin.com)
* **Telegam: ** [Adefe](https://t.me/Adefe)
